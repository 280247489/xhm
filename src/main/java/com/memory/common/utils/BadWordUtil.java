...
public class BadWordUtil {
    ...
    public static Map<String,String> wordMapTemp;
    public static Map<String,String> wordMapAll =new HashMap();
    public static int minMatchTYpe = 1;      //最小匹配规则
    public static int maxMatchType = 2;      //最大匹配规则
    public static final String CHECKPATH = "dics";

    //内存信息初始化
    public static  void initWords(){
        ArrayList<File> fileArrayList = FileUtils.getFiles(CHECKPATH);

        for (File file : fileArrayList) {
            BadWordUtil.words = readTxtByLine(file.getPath());
            BadWordUtil.wordMapAll.putAll( addBadWordToHashMap(BadWordUtil.words));
        }
    }
    ...
    private static  Map<String,String>  addBadWordToHashMap(Set<String> keyWordSet) {
        wordMapTemp = new HashMap(keyWordSet.size());     //初始化敏感词容器，减少扩容操作
        String key = null;
        Map nowMap = null;
    ...
    public static void main(String[] args) {
        ...
        String string = "判断文字1111,222,3333";
        System.out.println("待检测语句字数：" + string.length());
        long beginTime = System.currentTimeMillis();
        Set<String> set = BadWordUtil.getBadWord(string, 2);
        String returnStr = BadWordUtil.replaceBadWord(string,2,"*");

        long endTime = System.currentTimeMillis();
        System.out.println("语句中包含敏感词的个数为：" + set.size() + "。包含：" + set);
        System.out.println("总共消耗时间为：" + (endTime - beginTime));
        System.out.println("過濾后的文字為："+returnStr);
    }
}